...
def create_twitter_account(agent_id, username, name, twitter_user_id, api_key):
    ...
    if response.status_code == 200:
        ...
        print(f"Error creating Twitter account: {response.status_code} - {response.text}")
        return None
...
def create_interaction(agent_id, twitter_user_id, interaction_type, api_key, tweet_id=None, user_id=None):
    url = f"{BASE_URL}/agents/{agent_id}/accounts/{twitter_user_id}/interactions/"
    headers = {"access-token": api_key}
    payload = {"interaction_type": interaction_type, "tweet_id": tweet_id, "user_id": user_id}
    response = requests.post(url, json=payload, headers=headers)
    if response.status_code == 200:
        return response.json()
    else:
...
def simulate_user_behavior():
    # 1. Create an agent
    agent = create_agent(f"Simulated Agent")
    if not agent:
    ...
    for i in range(random.randint(1, 2)):
        ...
        account = create_twitter_account(agent_id, f"sim_user_{i+1}", f"Sim User {i+1}", twitter_user_id, api_key)
        if account:
            twitter_accounts.append({"agent_id": agent_id, "twitter_user_id": twitter_user_id, "user_name": account["name"]})
    ...
    for account in twitter_accounts:
        for i in range(random.randint(2, 4)):
            ...
            tweet = create_tweet(account["agent_id"], account["twitter_user_id"], account["user_name"], f"This is a simulated tweet {i+1}", created_at, api_key)

        for _ in range(random.randint(5, 10)):
            interaction_type = random.choice(interaction_types)
            create_interaction(account["agent_id"], account["twitter_user_id"], interaction_type, api_key)
