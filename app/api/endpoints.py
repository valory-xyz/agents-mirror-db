...
@router.post(
    "/api/agents/{agent_id}/twitter_accounts/", response_model=schemas.TwitterAccount
)
def create_twitter_account(
    ...
    existing_account = (
        ...
        .filter(models.TwitterAccount.twitter_user_id == account.twitter_user_id)
        .first()
    )
    if existing_account:
        raise HTTPException(
            status_code=409,
            detail="Twitter account with this twitter_user_id already exists",
        )
...
@router.post(
    "/api/agents/{agent_id}/accounts/{twitter_user_id}/tweets/",
    response_model=schemas.Tweet,
)
def create_tweet(
    agent_id: int,
    twitter_user_id: str,
...
@router.post(
    "/api/agents/{agent_id}/accounts/{twitter_user_id}/interactions/",
...
def create_interaction(
    ...
    if interaction.interaction_type == InteractionType.follow:
        if interaction.user_id is None:
            raise HTTPException(
                status_code=400, detail="user_id is required for follow interactions"
            )
    else:
        if interaction.tweet_id is None:
            raise HTTPException(
                status_code=400,
                detail="tweet_id is required for non-follow interactions",
            )

    if interaction.tweet_id is not None:
        db_tweet = (
            db.query(models.Tweet)
            .filter(models.Tweet.tweet_id == interaction.tweet_id)
            .first()
        )
        if db_tweet is None:
            db_tweet = models.Tweet(
...
@router.get(
    "/api/twitter_accounts/{twitter_user_id}/interactions/",
...
def get_interactions_by_twitter_user_id(
    ...
    db_interactions = (
        db.query(models.Interaction)
        .join(models.Tweet)
        .filter(models.Tweet.twitter_user_id == twitter_user_id)
        .all()
    )
    if not db_interactions:
        raise HTTPException(
            status_code=404, detail="No interactions found for this Twitter user ID"
        )
    return db_interactions


@router.get(
    "/api/agents/{agent_id}/twitter_accounts/",
...
def get_twitter_accounts(
    ...
    if not db_twitter_accounts:
        raise HTTPException(
            status_code=404, detail="No Twitter accounts found for this agent ID"
        )
    return db_twitter_accounts


@router.get(
    "/api/agents/{agent_id}/twitter_accounts/tweets/",
...
def get_latest_tweets_by_agent_id(
    ...
    db_tweets = (
        db.query(models.Tweet)
        .filter(models.Tweet.twitter_user_id.in_(twitter_user_ids))
    ...
    if not db_tweets:
        raise HTTPException(
            status_code=404,
            detail="No tweets found for the associated Twitter accounts",
        )
...
def get_interactions_by_agent_id(
    ...
    if not db_interactions:
        raise HTTPException(
            status_code=404, detail="No interactions found for this agent ID"
        )
    return db_interactions
...
@router.get(
    "/api/twitter_accounts/{twitter_user_id}", response_model=schemas.TwitterAccount
)
...
@router.get(
    "/api/agents/{agent_id}/attributes/{attr_def_id}",
...
@router.get(
    "/api/agents/{agent_id}/all-attributes/",
...
def read_all_attributes_of_agent(
    ...
    agent_attributes = (
        db.query(models.AgentAttribute)
        .filter(models.AgentAttribute.agent_id == agent_id)
        .all()
    )

    if not agent_attributes:
        raise HTTPException(
            status_code=404, detail="No attributes found for this agent ID"
        )
...
def update_agent_type(
    ...
    for key, value in agent_type.model_dump().items():
...
@router.post(
    "/api/agent-types/{type_id}/attributes/", response_model=schemas.AttributeDefinition
)
...
@router.get(
    "/api/agent-types/{type_id}/attributes/",
...
def read_attribute_definitions_by_type(
    ...
    attr_defs = (
        db.query(models.AttributeDefinition)
        .filter(models.AttributeDefinition.type_id == type_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def update_attribute_definition(
    ...
    for key, value in attr_def.model_dump().items():
...
@router.delete(
    "/api/attributes/{attr_def_id}", response_model=schemas.AttributeDefinition
)
...
def create_agent_registry(
    ...
    db_agent_type = (
        db.query(models.AgentType)
        .filter(models.AgentType.type_id == agent_registry.type_id)
        .first()
    )
    if db_agent_type is None:
        raise HTTPException(status_code=404, detail="Agent type not found")
    ...
    existing_agent = (
        db.query(models.AgentRegistry)
        .filter(models.AgentRegistry.eth_address == agent_registry.eth_address)
        .first()
    )
...
def update_agent_registry(
    ...
    for key, value in agent_registry.model_dump(exclude_unset=True).items():
...
def read_agents_by_type(
    ...
    agents = (
        db.query(models.AgentRegistry)
        .filter(models.AgentRegistry.type_id == type_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
@router.post(
    "/api/agents/{agent_id}/attributes/", response_model=schemas.AgentAttribute
)
def create_agent_attribute(
    ...
    if auth_agent_id != agent_id:
        print(
            f"Authorization failed: auth_agent_id {auth_agent_id} does not match agent_id {agent_id}"
        )
        raise HTTPException(
            status_code=403, detail="Not authorized to create attributes for this agent"
        )
    ...
    db_attr_def = (
        db.query(models.AttributeDefinition)
        .filter(models.AttributeDefinition.attr_def_id == agent_attr.attr_def_id)
        .first()
    )
    if db_attr_def is None:
        print(
            f"Attribute definition with attr_def_id {agent_attr.attr_def_id} not found"
        )
        raise HTTPException(status_code=404, detail="Attribute definition not found")
    ...
    print(
        f"Created agent attribute with attribute_id {db_agent_attr.attribute_id} for agent_id {agent_id}"
    )
    return db_agent_attr
...
@router.get(
    "/api/agent-types/{type_id}/attributes/{attr_def_id}/values",
    response_model=List[schemas.AgentAttribute],
)
def read_attribute_values_by_type(
    ...
    if db_attr_def is None:
        raise HTTPException(
            status_code=404, detail="Attribute definition not found for this agent type"
        )

    # Get all agents of the specified type
    agent_ids = (
        db.query(models.AgentRegistry.agent_id)
        .filter(models.AgentRegistry.type_id == type_id)
        .all()
    )
    agent_ids = [
        agent_id[0] for agent_id in agent_ids
    ]  # Extract agent_ids from result tuples
...
def update_agent_attribute(
    ...
    if auth_agent_id != db_agent_attr.agent_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to update this attribute"
        )

    for key, value in agent_attr.model_dump(exclude_unset=True).items():
        if value is not None:
            setattr(db_agent_attr, key, value)

    db_agent_attr.last_updated = datetime.datetime.now(datetime.timezone.utc)
    db.commit()
    db.refresh(db_agent_attr)
    return db_agent_attr
...
def delete_agent_attribute(
    ...
    if auth_agent_id != db_agent_attr.agent_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to delete this attribute"
        )
